...
class QurySerch(ttk.Frame):
    def __init__(self,parent,controller):
        ...
        self.columnname = ["المستخدم","المودع",'الفرز']
        ...
        FarmTree = ttk.Frame(pachSpent,width=900,height=400)
        ...
        treeview.column('#8', width=150,anchor='center')
        treeview.column('#9', width=150,anchor='center')
        treeview.column('#10', width=200,anchor='center')
    ...
    def loadqury():
        ...
        DatatPablic = StationMonthSqly.inComin_Pablic_drive() 
        #    وظيفة البحث حسب التاريخ 
    def Oprtionfun(self,k):
        global kind_oprtion
        if k == 'day':
            kind_oprtion.set(self.column[2])
        else:
            kind_oprtion.set(self.column[3])
    ...
    def on_search(self,event,d='name'):
            ...
            if seaction_select.replace(" ","").lower() == "الحسابات".replace(" ","").lower():
                for item in  DatatPablic:
                    listObject = (item[0],item[1],item[2],item[3],item[4],item[5],item[6],item[7])
                    # print(listObject)
    ...
    def Funextrct(self,ite,item):
        if kind_oprtion.get().lower().replace(" ","") == "الاسم وتاريخ الشهر واليوم".lower().replace(" ",""):
            if datetime.strptime(ite['Timedey'],'%Y-%m-%d').date() ==  datetime.strptime(select_entryD.get(),'%Y-%m-%d').date() :
                ...
                if datetime.strptime(ite['Timedey'],'%Y-%m-%d').month ==  datetime.strptime(str(select_entryMonth.get()),"%Y-%m").month and  datetime.strptime(ite['Timedey'],'%Y-%m-%d').year ==  datetime.strptime(str(select_entryMonth.get()),"%Y-%m").year:
                    # print(ite)
                    # print(kind_oprtion.get()) 
                    self.oprtion_all(ite,item)
