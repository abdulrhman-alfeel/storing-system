...
"""
        ...
        def Oprtion(kind):
            for pic in timeData:
                count = StationMonthSqly.get_message("yars",pic,kind,subMes['nameSorting'])
                dayDate = datetime.strptime(str(pic),'%Y-%m').strftime('%B')
                objectMonth = {'count':count[0][0],'month':switchMonth(dayDate),'oprtion':kind,"sort":subMes['nameSorting'],'DateTime':pic}
