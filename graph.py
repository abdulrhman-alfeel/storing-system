...
from data import data_function
...
class graph(ttk.Frame):
    def __init__(self,parent,controller):
        ...
        global inventory_day_data 
        global inventory_month_data
        global upper_frame
        global canva
        number = 0
        ...
        fram_entry.grid(row=0,column=0,padx=10,pady=10,
                        # sticky='nesw'
                        )
        ...
        select_kindcont.grid(row=0,column=2,padx=5,pady=15,
                            #  sticky='nesw'
                             )
...
# textDisplay = get_display(textReshaped)
    ...
    def graph_data(self):   
        ...
        if len(inventory_month_data) > 0 and len(inventory_day_data) > 0 :
           ...
           button_view = ttk.Button(lower_frame,text='عرض الرسم البياني',width=20, command=self.loden_graph)
           button_view.pack(side='bottom',fill='both',expand=True,padx=250,pady=250)
    ...
    def waitang_class():
        time.sleep(5)
    def loden_graph(self):
        button_view.config(text='الرجاء الانتظار' , command=self.waitang_class)
        button_view.configure(width='100%')
        button_view.update()
        time.sleep(2)
        global inventory_day_data
        global inventory_month_data
        inventory_day_data,inventory_month_data = data_function()
        lower_frame.destroy()
        # button_view.pack_forget()
        self.graph_data()
