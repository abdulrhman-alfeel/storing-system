...
def data_function ():
    ...
    try:
        ...
        for us in user:
            print(us[name[1]][0])
        #     datas[f"{name[1]}"] = len([item for item in us[name[1]] if datetime.strptime(item["datamovement"],'%Y-%m-%d %H:%M:%S.%f').month == datetime.now().month ])
            ...
            #     # print(item['datamovement'])
                try:
                    ...
                    # print(dataname)
                    dataday[f"{name_day}"] = len([i for i in us[name[1]] if  datetime.strptime(i["datamovement"],'%Y-%m-%d %H:%M:%S.%f').day ==  dataname.day])
                    month.append(dataname.month)
                    # month.append(datasMonth)
                except ValueError as e:
                    print(e)
                    # print(datasMonth)
            for count in month:
                datasMonth[f"{name_month}"] = len([i['ID'] for i in us[name[1]] if  datetime.strptime(i["datamovement"],'%Y-%m-%d %H:%M:%S.%f').month ==  count])
    ...
    return inventory_day_data,inventory_month_data